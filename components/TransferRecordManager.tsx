...
export function TransferRecordManager() {
  const { selectItems, accountResourceMessage } = useContext(AppContext);
  const {} = useContext(TronAccountContext);
  const {
    currentPage,
    total,
    size,
    ...
    <Space vertical spacing="medium" align="center">
      <Space align="center" className="w-full" wrap>
        ...
        <Select
          value={queryType}
          placeholder="转账类型"
          onChange={(value: any) => setQueryType(value)}
          style={{ width: 200 }}
        >
        ...
        <Select
          ...
          style={{ width: 200 }}
        >
          {Object.keys(TransferStatus).map((item) => (
            <Select.Option key={item} value={item}>
              {item}
            </Select.Option>
          ))}
        ...
        <Space vertical align="end" className="flex-grow">
          <Space >
          <Button type="secondary" onClick={resetQuery}>重置</Button>
      ...
      <Table
        resizable
        columns={createTransferRecordColumns(resourceConverter)}
